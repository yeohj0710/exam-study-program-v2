...
3. 즉시 뜨는 StudyForge 로딩 창이 준비 상태를 보여주고, 준비가 끝나면 브라우저가 자동으로 열립니다. 이미 실행 중이면 새 서버를 만들지 않고 기존 프로그램 창만 다시 엽니다.

루트 `exe`는 작은 부트스트랩 실행 파일입니다. 무거운 서버 실행 파일은 `프로그램 구성 파일\launcher\StudyForgeCore.exe`에 들어 있어, 더블클릭 직후부터 로딩 창이 먼저 보입니다.
검은 로그 창 없이 백그라운드에서 실행됩니다. 종료할 때는 화면 오른쪽 위의 `종료` 버튼을 누릅니다.
...
- `시험 자료 암기 프로그램.exe`: 더블클릭하면 즉시 로딩 창을 띄우는 Windows 실행 파일
